...
export function UserProfile({
  ...
  const [settings, setSettings] = useState({
    ...
    communityNotifications: currentUser?.settings?.communityNotifications ?? true,
  ...
  const handlePhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      toast.error(language === 'am' ? 'እባክዎ ምስል ፋይል ይምረጡ' : 'Please select an image file');
      return;
    }

    if (file.size > 5 * 1024 * 1024) { // 5MB limit
      toast.error(language === 'am' ? 'ፋይል መጠን ከ5MB መብለጥ አይችልም' : 'File size must be less than 5MB');
      return;
    }
    ...
    try {
      ...
      toast.success(language === 'am' ? 'ፎቶ በተሳካ ሁኔታ ተሸጋግሯል' : 'Photo uploaded successfully');
    } catch (error) {
  ...
  const handleSaveProfile = () => {
    try {
      ...
      toast.error(language === 'am' ? 'መገለጫ መዘመን አልተሳካም' : 'Failed to update profile');
    }
  };
  ...
  const handlePasswordChange = () => {
    ...
    if (passwordForm.newPassword !== passwordForm.confirmPassword) {
      toast.error(language === 'am' ? 'አዲስ የመክፈቻ ቃሎች አይዛመዱም' : 'New passwords do not match');
      return;
    }

    if (passwordForm.newPassword.length < 6) {
      toast.error(language === 'am' ? 'የመክፈቻ ቃል ቢያንስ 6 ቁምፊ ያስፈልጋል' : 'Password must be at least 6 characters');
      return;
    }
    ...
    try {
      ...
      toast.success(language === 'am' ? 'የመክፈቻ ቃል ተቀይሯል' : 'Password changed successfully');
    } catch (error) {
      toast.error(language === 'am' ? 'የመክፈቻ ቃል መቀየር አልተሳካም' : 'Failed to change password');
    }
  };
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex items-center space-x-6">
                <div className="relative">
                  <Avatar className="w-24 h-24">
                    <AvatarImage src={profileForm.avatar || currentUser?.avatar} />
                ...
                <div className="flex-1">
                  <h3 className="text-xl font-semibold">{profileForm.name || currentUser?.name}</h3>
                  <p className="text-gray-600">{currentUser?.email}</p>
                  <div className="flex items-center space-x-4 mt-2">
                    <Badge variant="secondary">
                      {currentUser?.role === 'admin' 
                        ? (language === 'am' ? 'አስተዳዳሪ' : 'Administrator')
                        : (language === 'am' ? 'አባል' : 'Member')
                      }
                    </Badge>
                    <span className="text-sm text-gray-500">
                      {language === 'am' ? 'አባልነት' : 'Member'}: {calculateMembershipDuration()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="email">{language === 'am' ? 'ኢሜይል' : 'Email'}</Label>
                  <Input
                    id="email"
                    type="email"
                ...
                <div className="space-y-2">
                  <Label htmlFor="phone">{language === 'am' ? 'ስልክ' : 'Phone'}</Label>
                  <Input
                    id="phone"
                ...
                <div className="space-y-2">
                  <Label htmlFor="birthDate">{language === 'am' ? 'የትውልድ ቀን' : 'Birth Date'}</Label>
                  <Input
                    id="birthDate"
                    type="date"
                ...
                <div className="space-y-2">
                  <Label htmlFor="occupation">{language === 'am' ? 'ሙያ' : 'Occupation'}</Label>
                  <Input
                    id="occupation"
                ...
                <div className="space-y-2">
                  <Label htmlFor="preferredLanguage">{language === 'am' ? 'ተመራጭ ቋንቋ' : 'Preferred Language'}</Label>
                  <Select
                    value={profileForm.preferredLanguage}
                    onValueChange={(value) => setProfileForm(prev => ({ ...prev, preferredLanguage: value }))}
                    disabled={!isEditing}
                  >
                    <SelectTrigger>
                      <SelectValue />
              ...
              <div className="space-y-2">
                <Label htmlFor="emergencyContact">{language === 'am' ? 'የአስቸኳይ ጊዜ ዕውቂያ' : 'Emergency Contact'}</Label>
                <Input
                  id="emergencyContact"
                  value={profileForm.emergencyContact}
                  onChange={(e) => setProfileForm(prev => ({ ...prev, emergencyContact: e.target.value }))}
                  disabled={!isEditing}
                  placeholder={language === 'am' ? 'ስም እና ስልክ ቁጥር' : 'Name and phone number'}
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                {isEditing && (
                  <div className="flex space-x-2">
                    <Input
                      placeholder={language === 'am' ? 'ፍላጎት ይጨምሩ' : 'Add an interest'}
                      onKeyPress={(e) => {
                        if (e.key === 'Enter') {
        ...
        <TabsContent value="settings" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>{language === 'am' ? 'ማሳወቂያ ምርጫዎች' : 'Notification Preferences'}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <Label>{language === 'am' ? 'ኢሜይል ማሳወቂያዎች' : 'Email Notifications'}</Label>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label>{language === 'am' ? 'የጸሎት ማስታወሻዎች' : 'Prayer Reminders'}</Label>
                  <p className="text-sm text-gray-500">
                    {language === 'am' 
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => setSettings(prev => ({ ...prev, prayerReminders: checked }))}
                />
              </div>

              <div className="flex items-center justify-between">
                <div>
                  <Label>{language === 'am' ? 'የኮርስ ዝማኔዎች' : 'Course Updates'}</Label>
                  <p className="text-sm text-gray-500">
                    {language === 'am' 
              ...
              <div className="flex items-center justify-between">
                <div>
                  <Label>{language === 'am' ? 'የማህበረሰብ ማሳወቂያዎች' : 'Community Notifications'}</Label>
                  <p className="text-sm text-gray-500">
                    {language === 'am' 
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => setSettings(prev => ({ ...prev, communityNotifications: checked }))}
                />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{language === 'am' ? 'የግላዊነት ቁጥጥር' : 'Privacy Controls'}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <Label>{language === 'am' ? 'መገለጫ ታይታ' : 'Profile Visibility'}</Label>
                <Select
              ...
              <div className="flex items-center justify-between">
                <Label>{language === 'am' ? 'ስልክ ቁጥር አሳይ' : 'Show Phone'}</Label>
                <Switch
        ...
        <TabsContent value="security" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>{language === 'am' ? 'የመክፈቻ ቃል ቀይር' : 'Change Password'}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>{language === 'am' ? 'የአሁኑ የመክፈቻ ቃል' : 'Current Password'}</Label>
                <div className="relative">
                  <Input
              ...
              <div className="space-y-2">
                <Label>{language === 'am' ? 'አዲስ የመክፈቻ ቃል' : 'New Password'}</Label>
                <div className="relative">
                  <Input
              ...
              <div className="space-y-2">
                <Label>{language === 'am' ? 'አዲስ የመክፈቻ ቃል አረጋግጥ' : 'Confirm New Password'}</Label>
                <div className="relative">
                  <Input
          ...
          <Card>
            <CardHeader>
              <CardTitle>{language === 'am' ? 'የሁለት-ደረጃ ማረጋገጫ' : 'Two-Factor Authentication'}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div>
                  <p>{language === 'am' ? 'ተጨማሪ ደህንነት ይጨምሩ' : 'Add extra security to your account'}</p>
                  <p className="text-sm text-gray-500">
                    {settings.twoFactorAuth 
                      ? (language === 'am' ? 'ዕርዳታ ንቁ ነው' : '2FA is enabled')
                      : (language === 'am' ? '2FA ንቁ አይደለም' : '2FA is not enabled')
                    }
                  </p>
                </div>
                <Switch
          ...
          <Card className="border-red-200">
            <CardHeader>
              <CardTitle className="text-red-600">{language === 'am' ? 'አደገኛ ቀጠና' : 'Danger Zone'}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div>
                  ...
                  <Button 
                    variant="destructive"
                    onClick={() => {
                      if (window.confirm(language === 'am' ? 'እርግጠኛ ኖት?' : 'Are you sure?')) {
                        onDeleteAccount();
                      }
                    }}
                  >
        ...
        <TabsContent value="activity" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle>{language === 'am' ? 'የእኔ ንቃቶች' : 'My Activities'}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between py-2">
                  <div className="flex items-center space-x-3">
                    <BookOpen className="w-5 h-5 text-blue-500" />
                    <span>{language === 'am' ? 'ኮርሶች ተመዝግቧል' : 'Courses Enrolled'}</span>
                  </div>
                ...
                <div className="flex items-center justify-between py-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <span>{language === 'am' ? 'የጸሎት ጥያቄዎች' : 'Prayer Requests'}</span>
                  </div>
                ...
                <div className="flex items-center justify-between py-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <span>{language === 'am' ? 'የተላኩ ይዘቶች' : 'Submitted Content'}</span>
                  </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{language === 'am' ? 'የመለያ መረጃ' : 'Account Info'}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex justify-between">
                  <span className="text-gray-600">{language === 'am' ? 'የተመዘገበበት ቀን' : 'Member Since'}:</span>
                  <span>{new Date(currentUser?.createdAt || Date.now()).toLocaleDateString()}</span>
                </div>
                
                <div className="flex justify-between">
                  <span className="text-gray-600">{language === 'am' ? 'የመጨረሻ ግብኝት' : 'Last Login'}:</span>
                  <span>{new Date(currentUser?.lastLogin || Date.now()).toLocaleDateString()}</span>
                </div>

                <div className="flex justify-between">
                  <span className="text-gray-600">{language === 'am' ? 'መለያ ሁኔታ' : 'Account Status'}:</span>
                  <Badge variant="secondary">
                    {currentUser?.status === 'active' 
                      ? (language === 'am' ? 'ንቁ' : 'Active')
                      : (language === 'am' ? 'እንቅልፍ' : 'Inactive')
                    }
                  </Badge>
                </div>
              </CardContent>
